...
public class CareCenterDataSource {
	...
	List<CareCenter> mVaccinationList = new ArrayList<CareCenter>();
	...
	public boolean deleteData(int eVaccineId) {
		this.open();
		try {
			mICareDatabase.delete(ICareSQLiteHelper.TABLE_CARECENTER,
					ICareSQLiteHelper.COL_CARECENTER_ID + "=" + eVaccineId, null);
		} catch (Exception ex) {
		...
		if (mCursor != null) {
			if (mCursor.moveToFirst()) {
				...
				do {
					...
					mVaccinationList.add(new CareCenter(id, name, date, time, profileId));
