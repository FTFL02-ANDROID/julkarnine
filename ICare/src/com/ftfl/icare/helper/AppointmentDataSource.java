...
public class AppointmentDataSource {
	...
	List<Appointment> mAppointmentList = new ArrayList<Appointment>();
	...
	public boolean insert(Appointment eAppointment) {
		...
		long check = mICareDatabase.insert(ICareSQLiteHelper.TABLE_APPOINTMENT,
				null, cv);
	...
	public boolean updateData(int eAppointmentId, Appointment eAppointment) {
		...
		int check = mICareDatabase.update(ICareSQLiteHelper.TABLE_APPOINTMENT,
				cv,
		...
		if (mCursor != null) {
			if (mCursor.moveToFirst()) {
				...
				do {
					...
					mAppointmentList.add(new Appointment(id, name, date, time,
